...
class TradingBot:
    ...
    def __init__(self, bot_name: str, config: Dict[str, Any]):
        ...
        self.config = config
        self.running = True
        self.stopped_by_telegram = False  # Флаг для отслеживания причины остановки
        ...
        self.intervals = config.get("intervals", {
            ...
            "db_status_check": 60  # НОВОЕ: проверка статуса в БД каждую минуту
        })
        ...
        self.stats = {
            ...
            "errors": 0,
            "sync_fixes": 0,
            "telegram_stops": 0,  # НОВОЕ
            "auto_trades_skipped": 0  # НОВОЕ: пропущенные автосделки из-за остановки
        }
    ...
    def _check_db_status(self) -> bool:
        """
        Проверить статус бота в базе данных.
        
        Returns:
            bool: True если бот должен продолжать работу, False если нужно остановиться
        """
        try:
            # Получаем текущий статус из БД
            bot = db.get_bot(self.bot_id)
            if not bot:
                print(ConsoleMessages.error(f"❌ Бот ID {self.bot_id} не найден в БД"))
                return False
            
            current_status = bot["status"]
            
            # Если статус изменился на "stopped" - останавливаемся
            if current_status != "active":
                print(ConsoleMessages.warning(f"\n⚠️ Обнаружено изменение статуса в БД: {current_status}"))
                print(ConsoleMessages.warning(f"🛑 Бот останавливается по команде из Telegram"))
                ...
                self.stopped_by_telegram = True
                self.stats["telegram_stops"] += 1
                return False
            
            return True
            
        except Exception as e:
            print(ConsoleMessages.error(f"❌ Ошибка при проверке статуса в БД: {e}"))
            # При ошибке продолжаем работу (fail safe)
            return True
        ...
        try:
            ...
            for alert in alerts:
                ...
                if alert["level"] == "CRITICAL":
                    ...
                    db.update_bot_status(self.bot_id, "stopped", alert["type"])
                else:
                ...
                try:
                    ...
                    if trade_details:
                        success = notifier.send_close_notification({
                            ...
                            "exit_price": float(trade_details["exit_price"]),
                            ...
                            "order_id": trade_details.get("exit_order_id", ""),
    ...
    def _execute_strategy(self):
        ...
        try:
            ...
            if signal == "up":
                ...
                self._open_position("buy", current_price, manual=False)  # Явно указываем, что это автоматически
                
            elif signal == "down":
                print(ConsoleMessages.sell_signal(self.symbol, current_price))
                self._open_position("sell", current_price, manual=False)  # Явно указываем, что это автоматически
    ...
    # ==================== ИЗМЕНЁННЫЙ МЕТОД ====================
    def _open_position(self, side: str, price: float, manual: bool = False):
        """
        Открыть позицию.
        ...
        Args:
            side: "buy" или "sell"
            price: Текущая цена
            manual: True если открытие по ручной команде (из Telegram)
        """
        try:
            # Для автоматических сделок проверяем статус
            if not manual:
                # Быстрая проверка статуса перед сделкой
                bot = db.get_bot(self.bot_id)
                if bot and bot["status"] != "active":
                    print(ConsoleMessages.warning(f"⏸️ Бот остановлен (статус: {bot['status']}), пропускаю автоматическую сделку"))
                    self.stats["auto_trades_skipped"] += 1
                    return
            
            quantity = self._calculate_quantity(price)
            tp_price, sl_price = self._calculate_tp_sl(price, side)
            
            # Определяем источник для order_manager
            source = "auto" if not manual else "manual"
            ...
            if result["success"]:
                ...
                source_text = "автоматически" if not manual else "вручную"
                print(f"  ✅ Позиция открыта {source_text}")
                ...
                notifier.send_trade_notification({
                    ...
                    "symbol_pnl": self.position_tracker.get_symbol_pnl(),
                    "total_pnl": self.position_tracker.get_total_pnl(),
                    "source": source  # Передаём источник в уведомление
                })
            else:
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                if self._should_run("db_status_check", current_time):
                    if not self._check_db_status():
                        print(ConsoleMessages.warning("\n🛑 Бот остановлен по команде из Telegram"))
                        break  # Выходим из цикла
                    self._update_last_action("db_status_check", current_time)
    ...
    def _cleanup(self):
        ...
        try:
            self._log_status()
            # Обновляем статус в БД, только если остановка не по Telegram
            # (потому что Telegram уже обновил статус)
            if not self.stopped_by_telegram:
                db.update_bot_status(self.bot_id, "stopped", "shutdown")
        except:
            pass
        ...
        print(f"   Остановок по Telegram: {self.stats['telegram_stops']}")
        print(f"   Пропущено автосделок (бот остановлен): {self.stats['auto_trades_skipped']}")
